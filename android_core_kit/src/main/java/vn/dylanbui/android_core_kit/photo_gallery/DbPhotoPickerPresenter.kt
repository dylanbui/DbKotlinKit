...
class DbPhotoPickerPresenter: MvpBasePresenter<DbPhotoPickerViewAction>() {
    ...
    fun openCamera(activity: Activity) {
        try {
            ...
            outputFileUri = Uri.fromFile(avataFile) // Default for older android
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                outputFileUri = FileProvider.getUriForFile(activity, activity.packageName + ".fileprovider",
                    avataFile!!)
            } else {
    ...
    private fun createImageFile(): File {
        ...
        try {
            ...
            if (!storageDir.exists())
                storageDir.mkdirs()
            image = File(storageDir.absolutePath + "/$imageFileName")
    ...
    fun onCapturePhotoResult(context: Context, requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        Log.e("TAG", "avataFile?.absolutePath = " + avataFile?.absolutePath)

        ifViewAttached { v -> v.addImageToFirstList(DbPhoto(avataFile?.absolutePath)) }

        // Khong xu ly fix xoay anh khi chup
        // Ham onCompressImageCompleted cung se khong chay
